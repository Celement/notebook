...
1225|  import lombok.extern.slf4j.Slf4j;
1226|  import org.springframework.amqp.core.ReturnedMessage;
```
...
安装完成后，我们访问 http://192.168.2.166:15672即可看到管理控制台。首次访问需要登录，默认的用户名和密码在配置文件中已经指定了。
登录后即可看到管理控制台总览页面：
其中包含几个概念：
...
这里的用户都是RabbitMQ的管理或运维人员。目前只有安装RabbitMQ时添加的`pp`这个用户。仔细观察用户表格中的字段，如下：

- ，也就是用户名
- `Name`：`pp`ministrator`，说明``用户是超级管理员，拥有所有权限
...
spring:
  rabbitmq:
    host: 192.168.2.166 # 你的虚拟机IP
    port: 5672 # 端口
    virtual-host: /pp # 虚拟主机
    username: pp # 用户名
    password: 123 # 密码
```
...
然后在`consumer`服务的`com.pp.consumer.listener`包中新建一个类`SpringRabbitListener`，代码如下：

```java
package com.pp.consumer.listener;
...
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_3】
...
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_5】
...
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_7】
...
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_11】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_13】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_15】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_17】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_19】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_21】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_23】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_25】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_27】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_29】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_31】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_33】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_35】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_37】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_39】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_41】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_43】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_45】
消费者2 收到了 work.queue的消息...... ：【hello, worker, message_47】
...
- 创建一个名为` pp.fanout`的交换机，类型是`Fanout`
- 创建两个队列`fanout.queue1`和`fanout.queue2`，绑定到交换机`pp.fanout`
...
1.  声明一个名为`pp.direct`的交换机
2.  声明队列`direct.queue1`，绑定`pp.direct`，`bindingKey`为`blud`和`red`
3.  声明队列`direct.queue2`，绑定`pp.direct`，`bindingKey`为`yellow`和`red`
4.  在`consumer`服务中，编写两个消费者方法，分别监听direct.queue1和direct.queue2 
5.  在publisher中编写测试方法，向`pp.direct`发送消息 
...
然后声明一个direct类型的交换机，命名为`pp.direct`:

然后使用`red`和`blue`作为key，绑定`direct.queue1`到`pp.direct`：

同理，使用`red`和`yellow`作为key，绑定`direct.queue2`到`pp.direct`，步骤略，最终结果：
...
public void testSendDirectExchange() {
    // 交换机名称
    String exchangeName = "pp.direct";
    // 消息
    String message = "地震，惊现哥斯拉！";
    // 发送消息
...
public void testSendDirectExchange() {
    ...
    String exchangeName = "pp.direct";
    // 消息
    String message = "最新报道，哥斯拉是从海上出来的！";
    // 发送消息
...
```java
package com.pp.consumer.config;
...
public class FanoutConfig {
    ...
    public FanoutExchange fanoutExchange(){
        return new FanoutExchange("pp.fanout");
    }
...
public class DirectConfig {
    ...
    public DirectExchange directExchange(){
        return ExchangeBuilder.directExchange("pp.direct").build();
    }
...
@RabbitListener(bindings = @QueueBinding(
    ...
    exchange = @Exchange(name = "pp.direct", type = ExchangeTypes.DIRECT),
    key = {"red", "blue"}
))
public void listenDirectQueue1(String msg){
    System.out.println("消费者1接收到direct.queue1的消息：【" + msg + "】");
...
@RabbitListener(bindings = @QueueBinding(
    ...
    exchange = @Exchange(name = "pp.topic", type = ExchangeTypes.TOPIC),
    key = "china.#"
))
public void listenTopicQueue1(String msg){
    System.out.println("消费者1接收到topic.queue1的消息：【" + msg + "】");
...
docker stop mq
```
然后测试发送一条消息，会发现会每隔2秒重试1次，总共重试了3次。消息发送的超时重试机制配置成功了！
...
具体如图所示：
![image-20240330135744822](MQ高级.assets/image-20240330135744822.png)
总结如下：
...
void testPublisherConfirm() {
    ...
    // 3.发送消息
    rabbitTemplate.convertAndSend("pp.direct", "q", "hello", cd);
}
```
执行结果如下：
![image-20240326080508426](MQ高级.assets/image-20240326080508426.png)
...
![image-20240326080710901](MQ高级.assets/image-20240326080710901.png)
设置为`Durable`就是持久化模式，`Transient`就是临时模式。

### 5.1.2.队列持久化
在控制台的Queues页面，添加队列时，同样可以配置队列的`Durability`参数：
![image-20240326080917270](MQ高级.assets/image-20240326080917270.png)
除了持久化以外，你可以看到队列还有很多其它参数，有一些我们会在后期学习。

### 5.1.3.消息持久化
在控制台发送消息的时候，可以添加很多参数，而消息的持久化是要配置一个`properties`：
![image-20240326081058104](MQ高级.assets/image-20240326081058104.png)
...
![image-20251212232719073](assets/image-20251212232719073.png)
...
![image-20240326081326593](MQ高级.assets/image-20240326081326593.png)
...
![image-20240326082100605](MQ高级.assets/image-20240326082100605.png)
放行以后，由于抛出的是**消息转换异常**，因此Spring会自动返回`reject`，所以消息依然会被删除：
![image-20240326082131168](MQ高级.assets/image-20240326082131168.png)
...
![image-20240326082342124](MQ高级.assets/image-20240326082342124.png)

放行以后，由于抛出的是业务异常，所以Spring返回`ack`，最终消息恢复至`Ready`状态，并且没有被RabbitMQ删除：
![image-20240326082855864](MQ高级.assets/image-20240326082855864.png)
...
![image-20240331110417010](rabbitmq.assets/image-20240331110417010.png)
