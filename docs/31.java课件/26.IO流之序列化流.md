...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121085423.png)
...
* public Object setProperty(String key, String value)调用 Hashtable 的方法
put。
...
>
>   把集合中的数据，保存到指定的流所对应的文件中，参数*commonts*代表对描述信息
>
>   5，关闭流
...
>
>   load(InputStream)
>   把指定流所对应的文件中的数据，读取出来，保存到*Propertie*集合中
>
>   load(Reader)
>
>   4,关闭流
...
ObjectOutputStream 将 Java 对象的基本数据类型和图形写入 OutputStream。可以使用
ObjectInputStream 读取（重构）对象。通过在流中使用文件可以实现对象的持久存储。

注意：只能将支持 java.io.Serializable 接口的对象写入流中

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121085424.png)

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121085425.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121085426.png)

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121085427.png)
...
FileUtils.copyFile(new File("D:\\test.avi"), new
File("D:\\copy.avi"));
...
FileUtils.copyDirectoryToDirectory(new File("D:\\基础班"), new
File("C:\\abc"));
...
-   方法
    ...
    -   writeStringToFile(File file，String
        content)：将内容content写入到file中；
