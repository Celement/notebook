...
>   Scanner类是引用数据类型的一种，我们可以使用该类来完成用户键盘录入，获取到录入的数  据。
>   Scanner使用步骤：
...
![QQ截图20200427211116](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140035.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140036.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140037.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140038.png)

上述代码中，变量num的值为19，模以2的结果为1，不等于0，判断条件不成立。因此会执行else后面{}中的语句，

### if…else if…else语句

if…else
if…else语句用于对多个条件进行判断，进行多种不同的处理。例如，对一个学生的考试成绩进行等级的划分，如果分数大于80分等级为优，否则，如果分数大于70分等级为良，否则，如果分数大于60分等级为中，否则，等级为差。if…else
if…else语句具体语法格式如下：
...
上述格式中，判断条件是一个布尔值。当判断条件1为true时，if后面{}中的执行语句1会执行。当判断条件1为false时，会继续执行判断条件2，如果为true则执行语句2，以此类推，如果所有的判断条件都为false，则意味着所有条件均未满足，else后面{}中的执行语句n+1会执行。if…else
if…else语句的执行流程如下图所示。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140039.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140040.jpg)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140041.png)

在上述代码中，x初始值为1，在满足循环条件x \<=
4的情况下，循环体会重复执行，打印x的值并让x进行自增。因此打印结果中x的值分别为1、2、3、4。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140042.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140043.jpg)

>   图1-12do…while循环的执行流程
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140044.png)

>   图1-13运行结果

我们发现WhileDemo.java与DoWhileDemo.java运行的结果一致，这说明什么呢？这说明do
…while循环和while循环能实现同样的功能。然而在程序运行过程中，这两种语句还是有差别的。如果循环条件在循环语句开始时就不成立，那么while循环的循环体一次都不会执行，而do…while循环的循环体还是会执行一次。若将DoWhileDemo.java中的循环条件x\<=4改为x
\<
1，DoWhileDemo.java运行结果会打印x=1，而WhileDemo.java运行结果什么也不会打印。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140045.png)
...
>   第三步，在第5行代码将j初始化为1，由于此时i的值为1，条件j \<=
>   i为true，首次进入内层循环的循环体，打印一个“\*”。
...
>   第九步，以此类推，在第3行会打印3个“\*”，逐行递增，直到i的值为10时，外层循环的判断条件i
>   \<= 9结果为false，外层循环结束，整个程序也就结束了。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140046.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140047.png)

>   图1-16运行结果

BreakDemo02.java与ForForDemo.java实现原理类似，只是在外层for循环前面增加了标记“itcast”。当i\>4时，使用break
pp;语句跳出外层循环。因此程序只打印了4行“\*”。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201118140048.png)

>   图1-17运行结果

上述代码中，使用for循环让变量i的值在1\~100之间循环，在循环过程中，当i的值为偶数时，将执行continue语句结束本次循环，进入下一次循环。当i的值为奇数时，sum和i进行累加，最终得到1\~100之间所有奇数的和，打印“sum
= 2500”。
...
> 3, 当语句体执行后， 执行控制条件表达式
>
> 4，接下来，回到第二步, 直到条件表达式为false
> 循环结束跳转语句（break、continue）
...
>   public int nextInt(int n) 返回一个，在
>   0（包括）和指定值（不包括）之间int随机数
