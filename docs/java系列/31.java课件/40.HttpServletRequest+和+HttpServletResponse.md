...
说白了也就是不管在哪个servlet里面，获取到的这个类的对象都是同一个。
...
![](assets/770066dc7a2970ea8af750692e66d204.png)

获取全局参数

![](assets/c8d9db93dd8d824297ae5e416c0dc3e0.png)
...
![](assets/78827b91268144df1d6a7b167b97e343.png)

### 通过classloader去获取web工程下的资源

![](assets/5b78ba07482b48f314a781b87af133e9.png)

### 使用ServletContext存取数据。

1.  定义一个登陆的html页面， 定义一个form表单

![](assets/d935c8639d06abc200037e3c5348a355.png)

1.  定义一个Servlet，名为LoginServlet

![](assets/28dedeae42ed11de6157075bc25d595c.png)

1.  针对成功或者失败，进行判断，然后跳转到不一样的网页

![](assets/61f01fe79db691f86915462c7f8af04c.png)

### ServletContext存取值分析

![](assets/71ab92249a96b767ef4102bfe6bff062.png)
...
只要在这个项目里面，都可以取。 只要同一个项目。 A项目 存，
在B项目取，是取不到的？ ServletContext对象不同。
...
(headerNames.hasMoreElements()) { String name = (String)
...
-   如果是GET方式
    ...
    1.  代码转码 String username = request.getParameter("username"); String
...
response.getWriter().write("我爱编程训练营...");
...
response.getOutputStream().write("我爱编程训练营..".getBytes("UTF-8"));
...
原因是tomcat里面有一个默认的Servlet -- DefaultServlet 。这个DefaultServlet
专门用于处理放在tomcat服务器上的静态资源。

![](assets/9fd9f314459343b8580559192687f405.png)
...
![](assets/f2d2b66e0b5c2dd7830b753ddd7101a6.png)
