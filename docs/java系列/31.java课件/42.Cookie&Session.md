...
针对浏览器类型，对文件名字做编码处理 Firefox (Base64) , IE、Chrome ...
使用的是URLEncoder
...
2. 请求次数最少有两次， 服务器在第一次请求后，会返回302 以及一个地址，
浏览器在根据这个地址，执行第二次访问。
...
5. 后续的请求，没法使用上一次的request存储的数据，或者
没法使用上一次的request对象，因为这是两次不同的请求。
...
![](assets/e10fccdb824a1c7eaf7e98ecabe349ca.png)
...
为了更好的用户体验，更好的交互
[自动登录]，其实从公司层面讲，就是为了更好的收集用户习惯[大数据]
...
![](assets/3545eb55ca9792739627a4440755f013.png)

-   获取客户端带过来的Cookie

-   //获取客户端带过来的cookie Cookie[] cookies = request.getCookies();
    if(cookies != null){ for (Cookie c : cookies) { String cookieName =
    c.getName(); String cookieValue = c.getValue();
...
2.  如果正确，则获取cookie。 但是得到的cookie是一个数组，
    我们要从数组里面找到我们想要的对象。
...
response.getWriter().write("欢迎您, "+userName +",上次来访时间是："+new
Date(lastVisitTime));
...
2.  在WebContent目录下新建一个jsp文件， product_list.jsp,
    然后拷贝原来product_list.html的内容到jsp里面。
    建好之后，jsp里面的所有ISO-8859-1 改成 UTF-8

-   拷贝html标签的所有内容。 替换jsp的html标签即可

1.  修改product_info.htm里面的手机数码超链接地址

-   [手机数码(current)](md/product_list.jsp)

1.  修改首页(index.html)顶部的手机数码跳转的位置为 product_list.jsp

-   [手机数码(current)](md/product_list.jsp)

### 分析

![](assets/febf3a13e51f47b88af0e07f042a41b2.png)
...
![](assets/62e4ec490ec8ef1b851a4f3661566687.png)
...
由于Cookie会保存在客户端上，所以有安全隐患问题。 还有一个问题，
...
session 是存放在服务器的内存中的一份数据。 当然可以持久化. Redis .
即使关了浏览器，session也不会销毁。
...
![](assets/e65a81393444afe3b0da4e48edcc55cc.png)
...
href='product_list.jsp'\>\<h3\>继续购物\</h3\>\</a\>\<br\>");

response.getWriter().write("\<a
href='cart.jsp'\>\<h3\>去购物车结算\</h3\>\</a\>");
