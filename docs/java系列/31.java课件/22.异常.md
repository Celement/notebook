...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084824.png)

查看API中Exception的描述，Exception 类及其子类是 Throwable
的一种形式，它用来表示java程序中可能会产生的异常，并要求对产生的异常进行合理的异常处理。

继续观察，我们可以发现Exception有继承关系，它的父类是Throwable。Throwable是Java
语言中所有错误或异常的超类，即祖宗类。

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084825.png)

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084826.png)

另外，在异常Exception类中，有一个子类要特殊说明一下，RuntimeException子类，RuntimeException及其它的子类只能在Java程序运行过程中出现。

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084827.png)

我们再来观察Throwable类，能够发现与异常Exception平级的有一个Error，它是Throwable的子类，它用来表示java程序中可能会产生的严重错误。解决办法只有一个，修改代码避免Error错误的产生。

![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084828.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084829.png)
...
>
> -   2,需要将这个异常对象告知给调用者。怎么告知呢？怎么将这个异常对象传递到调用者处呢？通过关键字throw就可以完成。throw
>     异常对象；
>
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084830.png)
...
throw new
ArrayIndexOutOfBoundsException("错误的角标，"+index+"索引在数组中不存在");
...
### try…catch…finally**异常处理的组合方式**

-   **try catch
    finally组合：**检测异常，并传递给catch处理，并在finally中进行资源释放。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201121084831.png)
