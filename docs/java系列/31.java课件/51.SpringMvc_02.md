...
<td><input name="name" value="<fmt:formatDate
...
限制此类下的所有方法请求url必须以请求前缀开头，通过此方法对url进行分类管理。
...
class="com.pp.ssm.controller.exceptionResolver.CustomExceptionResolver"/>
...
public @ResponseBody Items editItemSubmit_RequestJson(\@RequestBody Items
...
}/item/editItemSubmit_RequestJson.action",
...
`spring mvc 的<mvc:resources mapping=""`
`location="">实现对静态资源进行映射访问。`
...
<mvc:interceptor>
<mvc:mapping path="/**"/>
<bean class="com.pp.springmvc.filter.HandlerInterceptor1"></bean>
</mvc:interceptor>
<mvc:interceptor>
<mvc:mapping path="/**"/>
<bean class="com.pp.springmvc.filter.HandlerInterceptor2"></bean>
</mvc:interceptor>
</mvc:interceptors>
```
...
2.  有一个登录页面，需要写一个controller访问页面

3.  登录页面有一提交表单的动作。需要在controller中处理。
