...
API，可以为多种关系数据库提供统一访问，它由一组用Java语言编写的类和接口组成。是Java访问数据库的标准规范
...
![](media/256429c256d1c277050f414d8430b946.png)
...
```sql
INSERT INTO student(id,age,name) VALUES (1,22,"zs") 
```
...
​      ![image-20210926091423311](https://gitee.com/elementsong_admin/mdimage/raw/master/20210926091423.png)

​        

​       ![image-20210926091528355](https://gitee.com/elementsong_admin/mdimage/raw/master/20210926091528.png)

![image-20210926091631081](https://gitee.com/elementsong_admin/mdimage/raw/master/20210926091631.png)
...
需要连接数据库的位置（网址） user用户名 password 密码
...
此时，当用户输入正确的账号与密码后，查询到了信息则让用户登录。但是当用户输入的账号为XXX
密码为：XXX’ OR ‘a’=’a时，则真正执行的代码变为：
...
-   void **setXxx**(int index, Xxx xx)
    将指定参数设置为给定Java的xx值。在将此值发送到数据库时，驱动程序将它转换成一个
    SQL Xxx类型值。
...
public void demo01() throws Exception {
        ...
        Class.forName("com.mysql.jdbc.Driver");
       //获取链接
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fzstudent", "root", "123");    
       ...
       //获取预处理的对象
        ...
        preparedStatement.setString(3,"lisi");
...
在项目跟目录下，创建文件，输入“db.properties”文件名。  不需要加上字符串的形式
...
```java
        ...
        String s = studentArrayList.toString();
        System.out.println(s);          
```

建立一个学生表   然后自己写个学生类 jdbc的方式查询出来存到List集合中  并遍历该集合
