...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201115192619.png)
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201115192620.png)
...
![这里写图片描述](https://gitee.com/elementsong_admin/mdimage/raw/master/20201115192031.png)
...
![这里写图片描述](https://gitee.com/elementsong_admin/mdimage/raw/master/20201115192127.png)
...
![image-20210907141010872](https://gitee.com/elementsong_admin/mdimage/raw/master/20210907141017.png)

​             ![image-20210907141049813](https://gitee.com/elementsong_admin/mdimage/raw/master/20210907141049.png)

​            ![image-20210907141225451](https://gitee.com/elementsong_admin/mdimage/raw/master/20210907141225.png)
...
可是怎么才能不让访问呢？需要使用一个Java中的关键字也是一个修饰符
private(私有，权限修饰符)。只要将Person的属性和行为私有起来，这样就无法直接访问。
...
一般对成员属性的访问动作：赋值(设置 set)，取值(获取
get)，因此对私有的变量访问的方式可以提供对应的 setXxx或者getXxx的方法。
...
![](https://gitee.com/elementsong_admin/mdimage/raw/master/20201115192621.png)
...
2.  在堆内存中开辟空间，并为其分配内存地址0x1234，紧接着成员变量默认初始化(age =
    0)；将内存地址0x1234赋值给栈内中的Person p 变量

3.  继续执行p.setAge(30)语句，这时会调用setAge(int
    age)方法，将30赋值为setAge方法中的“age”变量；执行this.age =
    age语句，将age变量值30 赋值给成员变量this.age为30；
...
-   注意：
    ...
    -   上述代码中的 p.setAge(30)语句中，setAge(int
        age)方法中的this代表的就是p对象。

### this的应用

通过面向对象里面的 封装知识还有this  判断两个人年纪是否相同 
