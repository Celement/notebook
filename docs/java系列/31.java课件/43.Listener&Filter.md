...
![](assets/43f93699fe38f12777524ee35ed11c3c.png)
...
request ---httpServletRequest session ---httpSession aapplication ---
ServletContext
...
-   servletContext --- ServletContextAttributeListener
    ...
    ![](assets/8ec12d854ac74cc74308de891abd2d99.png)

-   request --- ServletRequestAttributeListener

    ![](assets/277ce82374cb13a541c6cf5450d2d833.png)

-   session --- HttpSessionAttributeListener

    ![](assets/7b226b2554aa7d8eec5d4e74dcdc7ca9.png)
...
那么可以考虑把session的值可以存储到硬盘上【钝化】，等下一次在使用的时候，在从硬盘上提取出来。
【活化】
...
D:\\tomcat\\apache-tomcat-7.0.52\\work\\Catalina\\localhost\\ListenerDemo\\pp
...
\<Store className="org.apache.catalina.session.FileStore" directory="pp"/\>
...
然后服务器派servlet处理。 在中间就可以过滤， 其实过滤器起到的是拦截的作用。
...
\<filter-class\>com.pp.filter.FilterDemo\</filter-class\>
...
2.  如果有多个过滤器， 那么他们会按照注册的映射顺序 来 排队。 只要有一个过滤器，
    不放行，那么后面排队的过滤器以及咱们的servlet都不会收到请求。

Filter细节：
------------

1.  init方法的参数 FilterConfig , 可以用于获取filter在注册的名字
    以及初始化参数。 其实这里的设计的初衷与ServletConfig是一样的。
...
-   REQUEST ： 只要是请求过来，都拦截，默认就是REQUEST FORWARD :
    只要是转发都拦截。 ERROR ： 页面出错发生跳转 INCLUDE ：
    包含页面的时候就拦截。
...
![](assets/66b4a47af7a993973f778aedb0e71f98.png)
    ...
    2.  有cookie
        ...
        -   /\*\*
            ...
            -   \@see Filter\#doFilter(ServletRequest, ServletResponse,
                FilterChain) \*/ public void doFilter(ServletRequest req,
...
ServletContextListner ： 应用被部署的时候，
服务器加载这个项目的时候，做一些初始化工作， 任务调度。 HttpSessionListener ：
统计在线人数 HttpSessionActivationListener ： 钝化活化处理
