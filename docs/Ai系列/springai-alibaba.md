...
![spring ai alibaba architecture.png](assets/spring-ai-alibaba-architecture.png)
...
您可以参考我们发布在官网的 [快速开始](https://java2ai.com/docs/1.0.0.2/get-started/chatbot/) 了解如何开发 Chatbot、智能体或工作流等应用，总的来说，根据不同场景，您可以选择使用 `ChatClient` 或 `Spring Ai Alibaba Graph` 两个核心组件来开发 AI 应用。
...
![spring ai alibaba playground.png](assets/playground.png)
...
- Spring AI 核心功能详解
  - [提示（Prompt）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#提示prompt)
  - [模型增强（The Augmented LLM）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#模型增强the-augmented-llm)
  - [顾问（Advisors）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#顾问advisors)
  - [检索（Retrieval）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#检索retrieval)
  - [记忆（ChatMemory）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#记忆chatmemory)
  - [工具（Tool）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#工具tool)
  - [评估（Evaluation）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#评估evaluation)
  - [可观测性（Observability）](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#可观测性observability)
  - 模型上下文协议（MCP）
    - [MCP 客户端](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#mcp-客户端)
    - [MCP 服务器](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#mcp-服务器)
  - [MCP 和安全](https://java2ai.com/blog/spring-ai-100-ga-released/?spm=5176.29160081.0.0.2856aa5c2PwbQU#mcp-和安全)
...
![spring ai alibaba ecosystem.png](assets/spring-ai-alibaba-ecosystem.png)
...
**工作流**是以相对固化的模式来人为的拆解任务，将一个大的任务拆解为一个固化的有多个分支的流程。工作流的优势是确定性强，模型作为流程中的一个结点起到的更多是一个分类决策的职责，因此它更适合意图识别等类别属性强的应用场景。工作流也有明显的劣势，它要求开发人员对业务流程有深刻的理解，整个流程是由人绘制的，模型在其中更多的只是内容生成、总结、分类识别的作用，并不能最大化利用模型的推理能力，因此很多人诟病这种模式是不够智能的。



用 Spring AI Alibaba Graph 可以轻松开发工作流，声明不同的结点，并将结点串联成一个流程图。

![spring ai alibaba workflow](assets/workflow.png)
...
![spring ai alibaba jmanus](assets/jmanus.png)
...
![architecture](assets/deepresearch.png)
...
访问Ollama官网 打开浏览器，访问Ollama的官方网站：https://ollama.ai/。 在官网首页，你可以找到“Download”按钮，点击后会跳转到下载页面。

选择Windows版本 在下载页面，找到适用于Windows系统的安装包。Ollama通常提供.exe格式的安装程序，直接点击下载即可。

https://ollama.ai/download

### 二、修改默认安装路径（默认C 盘）

Ollama 默认安装路径为 C:\Program Files\Ollama，如需指定其他 C 盘目录（如 D:\ollama），请按以下步骤操作：

1.以管理员身份打开命令提示符（Win+R → 输入 cmd → 右键选择 "以管理员身份运行"），切换到 OllamaSetup.exe 所在目录，或者用下面方式直接运行cmd
...
默认位置：C:\Users\%username%\.ollama\models

修改环境变量，在“新建系统变量”窗口中，输入变量名OLLAMA_MODELS，变量值为你希望的模型存储路径（例如D:\ollama\models）

![image-20251006000458407](assets/image-20251006000458407.png)

### 四、下载模型

官方模型仓库地址：https://ollama.com/search

例如搜索 qwen2.5

![image-20251006000514601](assets/image-20251006000514601.png)
